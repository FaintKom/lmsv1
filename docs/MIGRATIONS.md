...
❌ **Схема — только через миграции.** С 2026-07-19 (D2) fallback-слой
ALTER'ов в `_run_setup` удалён: lifespan гоняет `alembic upgrade head`
(пустая БД — `create_all` из моделей + `stamp head`). Изменение схемы без
миграции до прода не доедет. Rerun-safety (`IF NOT EXISTS`) в `op.execute`
всё ещё хороший тон — миграция может быть перезапущена после падения.
