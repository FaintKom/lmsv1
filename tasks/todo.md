...
- [x] Ручки, до которых #464 не дотянулся — PR #469, specs/063. Перебраны все
  44 маршрута `admin`-роутера; шесть считали «своих» по-старому. Проверка в
  проде добавила седьмое место и худшее: `/attendance/roster` отказывал
  учителю в его же группе, а карточка занятия рисовала отказ как «0/0, никто
  не зачислен». Заодно `POST /attendance` не проверял курс вовсе.
- [x] Остаток по всему бэкенду — specs/064. Поиск узкого признака
  (`course.teacher_id == user.id`) нашёл ещё три места: программа группы в
  журнале, взаимные проверки, разбор работы по тесту. Все три отказывали
  учителю, ведущему эту самую группу.

**Вопрос владельцу.** `courses/service.py:_check_course_owner` оставлен как
есть намеренно. Это право **править** курс, а не смотреть его. Правило
сформулировано про видимость («учитель видит только своих»); должен ли ведущий
группы получать доступ к редактированию содержимого чужого курса — решение
продуктовое, а не исправление ошибки.
