...
- [ ] **Ф0-1.** Переписать `marketing/target-segment.md` под новую нишу.
      Старую версию — в `marketing/archive/target-segment-2026-04.md` с
      пометкой, почему умерла (SAT снят с поверхности).
- [ ] **Ф0-2.** ICP-однушка (в том же файле): кто покупает (владелец/
      директор центра), размер (10–300 учеников), предметы, бюджет,
      кто платит, цикл решения.
...
- [ ] **Ф0-4.** Список возражений и ответы на них: «у нас уже Google
      Classroom», «а вы вообще живые», «а если вы закроетесь», «данные
      детей», «сколько времени на переезд».
- [ ] **Ф0-5.** Три отличия, которые проверяются в продукте за 60 секунд:
      автопроверка кода и математики (24 типа), AI-тьютор по уроку,
      родитель видит прогресс без звонков в администрацию.
...
- [ ] **Ф2-1.** Новый герой и тексты главной под Ф0-3. Через writing-skills,
      с показом того, что вырезано. Никаких выдуманных цифр и логотипов.
...
- [ ] **Ф2-3.** Публичное демо автопроверки **без логина**: пишешь код →
      сервер проверяет → видишь результат. Наш пруф вместо отзывов,
      которых нет.
...
**Статус: план утверждён, денег до прода не требует.** Провайдер —
...
- [ ] Новых зависимостей нет. `httpx` уже в проекте — тот же паттерн, что
      `knowledge/service.py` для Voyage
- [ ] `config.py`: `LLM_BASE_URL`, `LLM_MODEL`, `LLM_API_KEY` (все с ""
      по умолчанию). Пустой ключ → эндпоинты отдают 503, старт не падает
- [ ] `backend/app/common/llm.py` — одна `async def complete(system, user)
      -> str`, POST на `{base_url}/chat/completions`. JSON-схемы не нужны:
      единственный потребитель (фича A) хочет обычный текст
- [ ] Тесты: `complete` подменяется фикстурой. **CI никогда не ходит
      в сеть** — иначе прогоны начнут стоить денег
- [ ] Golden file: один живой ответ на free-модели сохраняется в
      `tests/fixtures/` и дальше играет роль модели. Реальный вызов нужен
      один раз, не на каждый прогон
...
## Песочница: доделать хардening (согласовано 2026-08-16)

Аудит песочницы. Все семь пунктов проверены по коду, порядок — мой, он
отличается от исходного: первый пункт там был назван усилением, а это
сломанная фича.

1. **Убрать `ulimit -v` из `sandbox/runner/executor.py`.** Проверено живьём в
   контейнере: `ulimit -v 262144` (это дефолт `memory_limit_mb: int = 256` из
   `backend/app/exercises/schemas.py:21`) не даёт стартовать ни JVM
   (`Could not reserve enough space for code cache`), ни рантайму Go
   (`failed to reserve page summary memory`). Оба языка в списке
   поддерживаемых. Реальный потолок и так даёт лимит контейнера (512M).
   Это баг в проде, а не хардening.
2. **Ограничить параллельность в раннере** (`sandbox/runner/main.py`).
   `/execute` принимает сколько угодно одновременных запусков на 2 CPU и
   512 МБ. Это ломает демо: класс из 20–25 человек жмёт «Запустить».
3. **`pids_limit`** во все четыре compose-файла. Форк-бомба — самая дешёвая
   атака, `ulimit -v` её не держит.
4. **Подключить `sandbox/policies/seccomp.json`** в `security_opt`. Профиль
   написан (`defaultAction: ALLOW`, `ERRNO` на socket/connect/bind/listen/
   accept), но нигде не указан. Не избыточен: `backend` тоже в `sandbox-net`
   (`docker-compose.prod.yml:62`), то есть `internal: true` закрывает выход в
   интернет, но не путь от кода ученика к внутреннему API.
5. **Убивать группу процессов по таймауту.** `proc.kill()` бьёт по шеллу,
   внуки выживают и продолжают жечь CPU — усугубляет пункт 2.
   `start_new_session=True` + `killpg`.
6. **`USER runner` в `sandbox/Dockerfile`.** Пользователь создаётся строкой 27,
   директивы нет — чужой код исполняется от root внутри контейнера.
7. **`exec` вместо `create_subprocess_shell`.** Инъекции нет (пути из
   `tempfile`), поэтому сам по себе не срочен — но становится бесплатным
   после пункта 1: без `ulimit`-префикса шелл больше не нужен.

Пункты 1 и 7 — одна правка. Пункты 2 и 5 — одна проблема с двух сторон.
Делать через spec-kit: затрагивает исполнение чужого кода, схему лимитов и
расход CPU.
