...
## Мобильная обрезка 2026-08-29 (specs/067)

Владелец открыл прод на телефоне и нашёл то, чего не видел замер: кнопка
«Новый курс» стояла на 67px за краем, а контейнер её обрезал, поэтому страница
даже не прокручивалась. Замер починен, обход повторён: было до −398px, стало
не больше −48px.

- [ ] **Дашборд аналитики на телефоне не собирается.** `dashboard-canvas.tsx`
  задаёт `COLS = 12` жёстко: на ширине 272px это ~11px на колонку, и плитки
  внутри виджетов ужимаются до 24px. Лечится мобильным режимом в одну колонку,
  но с оговоркой: `onLayoutChange` сохраняет раскладку, поэтому наивная правка
  приведёт к тому, что заход с телефона перезапишет дашборд, собранный на
  компьютере. Нужен режим только для чтения на узком экране.
  **Решение за владельцем** — риск в потере чужой раскладки, а не в вёрстке.
- [ ] **Хвосты до 48px.** Дата в журнале, подпись организации, адрес почты в
  профиле, карточка группы. Теряется хвост слова, а не орган управления, —
  поэтому в specs/067 сознательно не тронуты. Список воспроизводится командой
  `cd frontend && node e2e/mobile-audit.mjs`.

## Мобильный аудит 2026-08-28 (specs/065)

Обход всех страниц на стенде, три роли, 375×667. Вбок не разъезжается ничего.
Найдено и закрыто: 76 областей нажатия меньше 24×24, и главное — конструктор
аватара с редактором комнаты были на телефоне нерабочими: сетка отдавала всю
высоту 3D-виду, панели редактора оставался 1px.

- [ ] **Своя планка 44×44 не выполняется нигде — 1818 целей на 79 страницах.**
  [`docs/LMS_UX_DESIGN_GUIDE.md`](../docs/LMS_UX_DESIGN_GUIDE.md) требует
  44×44, фактический масштаб управляющих элементов 24–43. Дотянуть значит
  увеличить весь масштаб, а на экране 375px это заметно меньше содержимого в
  кадре. **Решение продуктовое — за владельцем.** Замер повторяется командой
  `cd frontend && node e2e/mobile-audit.mjs`.
