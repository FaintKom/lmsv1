...
- [ ] **Ф1-1. РЕШЕНИЕ ВЛАДЕЛЬЦА: юрлицо.** ИП в Грузии не зарегистрировано
      (`project_monetization_status`). Варианты: (а) зарегистрировать,
      (б) Merchant of Record — LemonSqueezy/Paddle продаёт от своего имени.
      ⚠️ Любой вариант = трата денег. Claude не выбирает и не платит.
- [ ] **Ф1-2.** Платёжный провайдер под выбранное в Ф1-1. В коде уже есть
      и Stripe (`billing/service.py`), и LemonSqueezy (`billing/ls_service.py`,
      `lemonsqueezy.py`) — дописывать с нуля ничего не надо, надо выбрать
      и включить.
...
- [ ] **Ф1-6.** Вернуть `/pricing`: снять редирект в `frontend/next.config.ts:7`,
      написать страницу. Итоговая сумма видна на странице — там, где Импульс
      её прячет, мы показываем.
...
- [ ] **Ф2-3.** Публичное демо автопроверки **без логина**: пишешь код →
      ...
      которых нет.
- [ ] **Ф2-4.** Форма «вопрос по внедрению» + прямой контакт в шапке.
      Школы без разговора не покупают.
