...
- [ ] «Кто отстаёт» на панели учителя. Данные уже считает `/journal/pacing`,
  нужен экран.
- [ ] «Журнал не заполнен» — занятия без отметок посещаемости.
- [ ] «Загрузка» у администратора: учителя и кабинеты, кто перегружен.
- [ ] Мигрировать оставшиеся места с узким `Course.teacher_id == user.id` на
  `app/common/teacher_scope.py` — по мере того, как трогаем их модули.
