...
**Goal:** Generalise current `lms` + `external lesson generator` + `external curriculum service` into a multi-subject, K12-through-university platform with template-first AI generation of plans, tasks, and lesson decks.
...
3. [`03-external lesson generator-pipeline.md`](03-external lesson generator-pipeline.md) — summary of `[external generator repo]`: deck generator with 24 layouts × 12 widgets, OpenRouter-based, client-side validator.
4. [`04-external curriculum service-graph.md`](04-external curriculum service-graph.md) — deep read of `[external curriculum repo]`: skill graph, BKT (4-param + EM calibration), Q-matrix, LessonFixture composer, 200 skills G1-G3.
...
- **Cloud LLM only** for generation — current Hetzner CX22 (2 vCPU, 3.7 GB RAM, no GPU) cannot host any useful local LLM for lesson generation. External lesson generator team already chose this path.
...
- **External curriculum service is production-grade.** Take BKT, EM calibration, SkillGraph, LessonFixture, versioning as-is. Generalise the math-specific bits (CRA, ModuleSection enum, Representation enum) into subject plugins.

## Open questions for next session

- Do we fork `external curriculum service` into the lms monorepo, or treat it as a separate package consumed by lms? (Leaning toward monorepo with `packages/curriculum-models` shared.)
...
External (read-only):
- `[external generator repo]\PROJECT_OVERVIEW.md` — [external project] overview, team, stack, hronology
- `[external generator repo]\README.md` — deck generator quickstart
- `[external curriculum repo]\README.md` — curriculum service quickstart
- `[external curriculum repo]\curriculum\enums.py` — all enums (mastery labels, lesson types, task formats, ...)
- `[external curriculum repo]\curriculum\graph.py` — SkillGraph + walk algorithms
- `[external curriculum repo]\curriculum\bkt.py` — BKT update + label derivation
- `[external curriculum repo]\curriculum\models.py` — Pydantic models (Skill, TaskSeed, LessonFixture, Student, …)
- `[external curriculum repo]\curriculum\services\lesson.py` — compose_lesson_fixture
- `[external curriculum repo]\curriculum\services\attempts.py` — record_attempt + Q-matrix BKT update
