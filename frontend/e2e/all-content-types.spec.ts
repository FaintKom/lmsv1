...
test.beforeAll(async ({ playwright }) => {
  ...
  L.file_upload = (
    await api.createLesson(courseId, moduleId, {
      ...
      content: { instructions: "Upload a text file.", allowed_types: [".txt"], max_file_mb: 5 },
    })
  ).id;
...
test("file_upload: uploads a file and completes", async ({ page }) => {
  ...
  await page.locator('input[type="file"]').setInputFiles({
    name: "answer.txt",
    mimeType: "text/plain",
    buffer: Buffer.from("my submission\n"),
  });
  await expect(page.getByText("answer.txt")).toBeVisible({ timeout: 20_000 });
});
...
test("quiz: answers correctly and passes", async ({ page }) => {
  ...
  // Option buttons render a letter chip (A/B/C) + the option text. The
  // correct option is "4"; match the button whose text ends in 4 but not 22.
  await page
    .getByRole("button")
    .filter({ hasText: /(^|\s)4$/ })
    .first()
