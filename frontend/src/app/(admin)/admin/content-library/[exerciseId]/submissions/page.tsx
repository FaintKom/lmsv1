...
function SubmissionRow({
 ...
 <div>
 <div
 className="flex cursor-pointer items-center gap-4 px-6 py-3 transition-colors hover:bg-surface-2 "
 onClick={onToggle}
 >
 {isExpanded ? (
...
function CodeSubmissionDetail({ submission }: { submission: ExerciseSubmission }) {
 ...
 <span className="text-xs text-text-muted">{submission.execution_time_ms}ms</span>
 )}
 </div>
...
function FileSubmissionDetail({ submission }: { submission: ExerciseSubmission }) {
 ...
 className="ml-auto flex items-center gap-1.5 rounded-lg bg-success-soft px-3 py-1.5 text-xs font-medium text-primary transition-colors hover:bg-primary-soft "
 download
 >
