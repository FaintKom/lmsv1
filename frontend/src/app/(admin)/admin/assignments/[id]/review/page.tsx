...
export default function AssignmentReviewPage() {
  ...
  if (!assignment) {
    return <p className="text-slate-500 dark:text-slate-400">Assignment not found.</p>;
  }

  const statusBadge = (status: string) => {
    switch (status) {
      case "graded":
        return <span className="rounded-full bg-emerald-100 px-2.5 py-0.5 text-xs font-medium text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400">Graded</span>;
      case "submitted":
      ...
      case "late":
        return <span className="rounded-full bg-orange-100 px-2.5 py-0.5 text-xs font-medium text-orange-700 dark:bg-orange-500/20 dark:text-orange-400">Late</span>;
      default:
        return <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-600 dark:bg-white/10 dark:text-slate-400">{status}</span>;
    }
  };
      ...
      <Card className="mb-6">
        <CardContent className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold text-slate-900 dark:text-slate-100">{assignment.title}</h1>
            <div className="mt-0.5 flex flex-wrap items-center gap-x-4 text-xs text-slate-500 dark:text-slate-400">
          ...
          <div className="text-right">
            <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">{submissions.length}</p>
              ...
              <Card key={sub.id} className={`border-l-4 ${sub.status === "graded" ? "border-l-emerald-400" : "border-l-blue-400"}`}>
                <CardContent className="p-0">
                  ...
                  <button
                    ...
                    className="flex w-full items-center gap-4 px-6 py-4 text-left transition-colors hover:bg-slate-50 dark:hover:bg-white/5"
                  >
                  ...
                  {isExpanded && (
                    <div className="border-t border-slate-100 px-6 py-4 dark:border-white/10">
                      ...
                      {sub.content && (
                        <div className="mb-4">
                          <p className="mb-1 text-xs font-medium text-slate-500 dark:text-slate-400">Answer</p>
                          <div className="whitespace-pre-wrap rounded-lg bg-slate-50 p-3 text-sm text-slate-700 dark:bg-white/5 dark:text-slate-300">
                            {sub.content}
                          </div>
                        </div>
                      )}
                      {sub.original_filename && (
                        <div className="mb-4">
                          <p className="mb-1 text-xs font-medium text-slate-500 dark:text-slate-400">Attached File</p>
                          <a
                            href={`${process.env.NEXT_PUBLIC_API_URL || ""}/api/v1/assignments/${assignmentId}/submissions/${sub.id}/file`}
                            className="inline-flex items-center gap-1.5 rounded-lg bg-slate-50 px-3 py-2 text-sm text-indigo-600 hover:bg-slate-100 dark:bg-white/5 dark:text-indigo-400 dark:hover:bg-white/10"
                            target="_blank"
                        ...
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
                          <div className="flex-shrink-0">
                            ...
                            <input
                              ...
                              className="w-24 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 focus:border-indigo-500 focus:outline-none dark:border-white/10 dark:bg-[#2C2C2C] dark:text-slate-100"
                            />
                          </div>
                          <div className="flex-1">
                            <label className="mb-1 block text-xs text-slate-500 dark:text-slate-400">Feedback</label>
                            <input
                              type="text"
                              value={g.feedback}
                              onChange={(e) =>
