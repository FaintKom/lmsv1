...
function ScoreTrendChart({ data }: { data: { date: string; score: number; mode: string }[] }) {
 ...
 <text x={p.x} y={p.y - 10} textAnchor="middle" className="fill-slate-600 text-[10px] font-bold">
 {p.score}
 </text>
 </g>
 ))}
...
function DomainBars({ stats }: { stats: Record<SATDomain, { correct: number; total: number; percent: number }> }) {
 ...
 <span className="font-medium text-text-muted ">{DOMAIN_LABELS[d]}</span>
...
export default function SATAnalyticsPage() {
 ...
 <p className="text-2xl font-bold text-ink-700 ">{s.value}</p>
 ...
 {weakDomains.map((d) => (
 <div key={d} className="flex items-center gap-3 rounded-lg bg-sun-50 px-4 py-2 ">
 <div className="h-2.5 w-2.5 rounded-pill" style={{ backgroundColor: DOMAIN_COLORS[d] }} />
 <span className="text-sm text-ink-700 ">{DOMAIN_LABELS[d]}</span>
 <span className="ml-auto text-xs font-bold text-warning-fg ">
 ...
 <span className="text-sm font-bold text-primary ">{r.scaledScore}</span>
 </div>
