...
export default function MathEditor({ config, onConfigChange }: MathEditorProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Template selector */}
      <div>
        <label className="mb-2 block text-xs font-medium text-ink-500 dark:text-ink-400">
          Template Type
        </label>
      ...
      {showPreview && (
        <div className="rounded-xl border border-ink-200 bg-ink-50 p-4 dark:border-white/10 dark:bg-[#161622]">
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="mb-1 block text-xs text-ink-500">Grid Range (±)</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Target Points</label>
        {points.map((p, i) => (
          <div key={i} className="mb-1.5 flex items-center gap-2">
            ...
            <input type="number" value={p.x} placeholder="x"
              ...
              className="w-20 rounded border border-ink-200 px-2 py-1 text-sm dark:border-white/10 dark:bg-[#1E1E1E] dark:text-ink-200" />
            ...
            {points.length > 1 && (
              <button onClick={() => onChange({ ...config, target_points: points.filter((_, j) => j !== i) })}
                className="text-xs text-coral-500 hover:text-coral-700">&times;</button>
            )}
          </div>
        ))}
...
function NumberLineConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Tick Interval</label>
...
function FractionsConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div className="grid grid-cols-3 gap-3">
      <div>
        <label className="mb-1 block text-xs text-ink-500">Numerator</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Denominator</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Display</label>
...
function EquationBalanceConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="mb-1 block text-xs text-ink-500">Left Side (comma-sep)</label>
        ...
        <div>
          <label className="mb-1 block text-xs text-ink-500">Right Side Fixed (comma-sep)</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Available Terms (JSON array)</label>
        <input type="text"
          ...
          className="w-full rounded-lg border border-ink-200 bg-white px-3 py-2 font-mono text-xs dark:border-white/10 dark:bg-[#1E1E1E] dark:text-ink-200" />
      </div>
    </div>
  );
}
...
function ArithmeticPuzzleConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div>
      <label className="mb-1 block text-xs text-ink-500">Equations (JSON)</label>
      <textarea
...
function FunctionGraphConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
      <div>
        <label className="mb-1 block text-xs text-ink-500">Function Type</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Target Params (JSON)</label>
...
function EquationSolverConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div className="space-y-3">
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Final Answer</label>
...
function MCMathConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div className="space-y-3">
      <div>
        <label className="mb-1 block text-xs text-ink-500">Question</label>
        <textarea rows={2} value={(config.question as string) || ""} onChange={(e) => onChange({ ...config, question: e.target.value })}
          className="w-full rounded-lg border border-ink-200 bg-white px-3 py-2 text-sm dark:border-white/10 dark:bg-[#1E1E1E] dark:text-ink-200" />
      </div>
      <div>
        <label className="mb-1 block text-xs text-ink-500">Choices (JSON)</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Explanation</label>
...
function NumericInputConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="mb-1 block text-xs text-ink-500">Correct Answers (comma-sep)</label>
...
function ScatterPlotConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-3 gap-3">
        <div>
          <label className="mb-1 block text-xs text-ink-500">Mode</label>
        ...
        <div>
          <label className="mb-1 block text-xs text-ink-500">Target Slope</label>
        ...
        <div>
          <label className="mb-1 block text-xs text-ink-500">Target Intercept</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Data Points (JSON)</label>
...
function TwoWayTableConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="mb-1 block text-xs text-ink-500">Row Headers (comma-sep)</label>
        ...
        <div>
          <label className="mb-1 block text-xs text-ink-500">Column Headers (comma-sep)</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Cells (JSON, null = blank)</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Answers (JSON: {"{"}&quot;r0c1&quot;: 13, ...{"}"})</label>
...
function CardSortConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div className="space-y-3">
      <div>
        <label className="mb-1 block text-xs text-ink-500">Categories (JSON)</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Cards (JSON: [{"{"} id, text, category {"}"}])</label>
...
function TablePatternConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="mb-1 block text-xs text-ink-500">X Values (comma-sep)</label>
        ...
        <div>
          <label className="mb-1 block text-xs text-ink-500">Y Values (comma-sep, ? for blank)</label>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="mb-1 block text-xs text-ink-500">Rule Label</label>
        ...
        <div>
          <label className="mb-1 block text-xs text-ink-500">Rule Answer</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Answers (JSON: {"{"} index: value {"}"})</label>
...
function InequalityConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      <div>
        <label className="mb-1 block text-xs text-ink-500">Slope</label>
        <input type="number" step={0.5} value={(config.slope as number) ?? 1} onChange={(e) => onChange({ ...config, slope: parseFloat(e.target.value) })}
          className="w-full rounded-lg border border-ink-200 bg-white px-3 py-2 text-sm dark:border-white/10 dark:bg-[#1E1E1E] dark:text-ink-200" />
      </div>
      <div>
        <label className="mb-1 block text-xs text-ink-500">Intercept</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Operator</label>
...
function GraphTransformConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      <div>
        <label className="mb-1 block text-xs text-ink-500">Parent Function</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Target H shift</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Target V shift</label>
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Target stretch (a)</label>
...
function VennConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div className="space-y-3">
      ...
      <div>
        <label className="mb-1 block text-xs text-ink-500">Answers (JSON)</label>
  ...
  return (
    <div>
      ...
      <p className="mb-2 text-xs text-ink-400">
        Use <code className="bg-ink-100 px-1 rounded dark:bg-white/10">window.LMS.reportResult({"{"} passed: true, score: 1.0 {"}"})</code> to submit the result.
      </p>
      <div className="h-[300px] rounded-lg border border-ink-200 overflow-hidden dark:border-white/10">
        <Editor
          height="100%"
          language="html"
          value={html}
  ...
  if (templateType === "custom_html") {
    ...
    return (
      <iframe srcDoc={srcdoc} sandbox="allow-scripts"
        className="h-[300px] w-full rounded-lg border border-ink-200 dark:border-white/10" title="Preview" />
    );
  }

  const template = MATH_TEMPLATES[templateType];
  if (!template?.component) return <p className="text-sm text-ink-400">No preview available</p>;

  const TemplateComponent = template.component;
  return (
    <Suspense fallback={<div className="flex items-center justify-center py-8"><Loader2 className="h-4 w-4 animate-spin text-ink-400" /></div>}>
