...
/** Tags whose text is code, not prose — a `$` in there is a dollar sign. */
const MATH_SKIP_TAGS = /^(code|pre|script|style)$/i;

/** Minimal entity decode so `x &lt; y` reaches KaTeX as `x < y`. */
...
function renderTex(tex: string, displayMode: boolean): string {
 try {
  return katex.renderToString(decodeEntities(tex), { displayMode, throwOnError: false });
 } catch {
  // Leave the source visible rather than swallowing the expression.
  const d = displayMode ? "$$" : "$";
  return `${d}${tex}${d}`;
 }
}
...
function renderMathInText(text: string): string {
 ...
 // Block math first, so `$$…$$` is not eaten by the inline pattern.
 let out = text.replace(/\$\$([\s\S]+?)\$\$/g, (_, tex) => renderTex(tex, true));
 // Inline delimiters must hug non-space characters. That is the LaTeX
 // convention and the only reliable way to tell `$x$` from prices such as
 // "$2 each and oranges for $3".
 out = out.replace(/\$(\S[^$\n]*?\S|\S)\$/g, (_, tex) => renderTex(tex, false));
 return out;
}

/**
 * Replace `$…$` / `$$…$$` with KaTeX markup **inside the HTML string**.
 *
 * This used to run as an effect that walked text nodes and swapped them in the
 * live DOM. That worked exactly once: any later re-render of a surrounding
 * component — HighlightableContent loads its highlights right after mount —
 * makes React reset the container from `dangerouslySetInnerHTML`, wiping the
 * rendered math, and the effect never re-runs because its input string has not
 * changed. Building the markup into the string removes the race entirely.
 *
 * Runs identically on the server and the client, so there is no hydration
 * mismatch. Called after sanitisation: the author's HTML is cleaned first, and
 * the KaTeX markup added afterwards is generated by KaTeX, which escapes its
 * own input.
 */
export function renderMathInHtml(html: string): string {
 ...
 let out = "";
 let skipDepth = 0;
 // Split into tags and the text between them; never look for math inside a tag.
 for (const part of html.split(/(<[^>]+>)/)) {
  if (part.startsWith("<")) {
   const match = /^<\s*(\/?)\s*([a-zA-Z0-9-]+)/.exec(part);
   if (match && MATH_SKIP_TAGS.test(match[2])) {
    skipDepth = match[1] === "/" ? Math.max(0, skipDepth - 1) : skipDepth + 1;
   }
   out += part;
  } else {
   out += skipDepth > 0 ? part : renderMathInText(part);
  }
 }
 return out;
}

/** Render HTML with KaTeX math support. */
function HtmlWithMath({ html }: { html: string }) {
 const normalized = useMemo(
  () => renderMathInHtml(sanitizeHtml(normalizeLessonHtml(html))),
  [html]
 );

 return (
  <div className="lms-lesson-content" dangerouslySetInnerHTML={{ __html: normalized }} />
 );
}
