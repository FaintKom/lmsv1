...
export function AiTutorPanel({ context }: AiTutorPanelProps) {
    ...
    return (
      ...
      >
        ...
        <button
          ...
          className="absolute -top-2 -right-2 z-[99] flex h-5 w-5 items-center justify-center rounded-full bg-ink-700/80 text-white opacity-0 transition-opacity group-hover:opacity-100 hover:bg-coral-500"
        ...
        <button
          ...
          className={`flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-green-500 to-green-600 text-white shadow-lg shadow-green-500/30 transition-all hover:shadow-xl hover:shadow-green-500/40 ${
  ...
  return (
    <div className="fixed bottom-20 right-2 left-2 z-[98] flex max-h-[75vh] flex-col overflow-hidden rounded-2xl border border-ink-200/80 bg-white shadow-2xl shadow-black/10 dark:border-white/10 dark:bg-[#2C2C2C] dark:shadow-black/30 md:bottom-4 md:left-auto md:right-6 md:h-[540px] md:w-[380px] md:max-h-none">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-ink-200/60 bg-gradient-to-r from-green-500 to-green-600 px-4 py-3 dark:border-white/10">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center px-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-green-100 to-green-100 dark:from-green-900/30 dark:to-green-900/30 mb-3">
      ...
      {remainingMessages !== null && remainingMessages <= 10 && (
        <div className="px-4 py-1 text-center text-xs text-sun-500 dark:text-sun-400">
          {t("ai.remaining").replace("{count}", String(remainingMessages))}
        </div>
      )}

      {/* Input */}
      <div className="border-t border-ink-200/60 p-3 dark:border-white/10">
        <div className="flex items-end gap-2">
          <textarea
            ...
            rows={1}
            className="flex-1 resize-none rounded-xl border border-ink-200 bg-ink-50 px-3 py-2.5 text-sm text-ink-900 placeholder-ink-400 focus:border-green-400 focus:outline-none focus:ring-2 focus:ring-green-400/20 dark:border-white/10 dark:bg-[#1E1E1E] dark:text-ink-100 dark:placeholder-ink-500 dark:focus:border-green-500"
            style={{ maxHeight: 80 }}
      ...
      <div
        className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed ${
          isUser
            ...
            : "bg-ink-100 text-ink-900 dark:bg-white/5 dark:text-ink-200"
        }`}
      >
        {isEmpty && isStreaming ? (
