...
export default function SATResults({
 ...
 {/* Hero score */}
 <div className="rounded-lg bg-gradient-to-br from-green-600 to-emerald-600 p-8 text-center text-white">
 ...
 <span className="font-medium text-text-muted ">{DOMAIN_LABELS[domain]}</span>
 ...
 <summary className={`flex cursor-pointer items-center gap-3 rounded-lg border px-4 py-3 transition-colors ${
 r.correct
 ...
 r.correct ? "bg-primary" : "bg-danger"
 }`}>
 {i + 1}
 </span>
 {r.module && (
 ...
 {r.correct ? <CheckCircle className="h-4 w-4 text-primary shrink-0" /> : <XCircle className="h-4 w-4 text-danger-fg shrink-0" />}
