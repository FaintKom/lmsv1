# Target Segment — Analysis and Recommendation (ARCHIVED)

> **Archived 2026-08-17. Do not use.** This document bet everything on
> test-prep centres with SAT Math as the wedge. The wedge is gone: SAT came off
> the product's surface on 2026-08-02, and `/sat-practice` now redirects to
> `/dashboard`. A recommendation whose central asset has been removed is not a
> stale document, it is a wrong one.
>
> It also carries a number that was never true. Segment B credits the product
> with "37 languages"; the sandbox runs five. PR #284 corrected the same figure
> in the legal pages. It is left unedited here so the error stays visible
> instead of quietly disappearing.
>
> Current positioning: [`../target-segment.md`](../target-segment.md).
