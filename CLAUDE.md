...
Чего worktree **не** разводит: порты хоста (`docker-compose.yml` — 5432, 8000,
3000; `docker-compose.qa.yml` — 8000, 3000, 8101), postgres под pytest,
`node_modules` и venv. Значит стек поднят только в одном worktree за раз, pytest
гоняется по одному, а новому worktree нужны свой `npm ci` и скопированные руками
