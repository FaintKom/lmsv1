...
**Два чата одновременно — каждому свой worktree.** Ветка принадлежит каталогу,
...
Первый чат остаётся в `F:\lms2.9`. Каждый следующий начинается словами
«работай в worktree» — по-другому Claude его не создаст. Получается
`.claude/worktrees/<имя>` со своей веткой от `origin/main` и своим HEAD; путь
в `.gitignore`, в репозиторий не протекает.
