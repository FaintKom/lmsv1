...
### Скиллы: вызывать до кода, не после

Spec Kit отвечает на «что и как», скиллы — на «как принято здесь». И то и другое
до первой строки кода, а не в ревью.

| Что трогаем | Что вызвать |
|---|---|
| Бэкенд, API, модели, миграции | `grasslms-backend` |
| Фронтенд, компоненты, страницы | `grasslms-frontend` |
| Интерфейс, поток, доступность | `ui-ux-pro-max` и `docs/LMS_UX_DESIGN_GUIDE.md` |
| Анимация, 3D, визуал | см. память `reference_motion_3d_skills.md` |
| Любой текст, который прочтёт человек | скиллы письма: `style-clarity-and-grace`, `avoid-ai-writing`, `beautiful-prose`, для русского ещё `ru-deslop` |

Правило распространяется на правки, а не только на новые фичи: строка в
существующем компоненте — это тот же фронтенд, с теми же конвенциями. Ограничения
проекта важнее умолчаний скилла: если скилл советует одно, а `frontend/CLAUDE.md`
или дизайн-гайд другое — прав проект.
...
**Каждый чат — в своём worktree, без исключений.** Ветка принадлежит каталогу,
а не сессии: HEAD, индекс и рабочее дерево одни на весь чекаут. Когда один агент
делает `git checkout`, ветка второго меняется под ним, молча. Разные PR не
спасают — PR режется от ветки, а ветка и есть общее состояние.

Так 2026-08-17 спек-коммит одной сессии уехал в прод внутри чужого PR #313
(«chore: pin node to one version»): коммит лежал на локальном `main`, вторая
сессия отвела от него ветку, и squash забрал всё, что не было upstream. Там был
только markdown, поэтому ничего не сломалось, но в тело PR это не попало и ревью
не проходило.

Поэтому worktree заводит **каждый** чат, включая первый, и заводит его до первой
правки. `F:\lms2.9` стоит на `main` и ничьей рабочей веткой не бывает.
Получается `.claude/worktrees/<имя>` со своей веткой от `origin/main` и своим
HEAD; путь в `.gitignore`, в репозиторий не протекает.

Правило действует с 2026-08-31 и заменяет прежнее «первый чат остаётся в
`F:\lms2.9`». Прежнее стоило столкновения в тот же день: чат собирал курс в
основном каталоге, второй отвёл там ветку `git checkout -b`, и HEAD первого
уехал с `main` молча — ни сообщения, ни конфликта.

Эта строка в `CLAUDE.md` нужна ещё и технически: `EnterWorktree` отказывается
работать, пока об этом не просит пользователь или инструкции проекта. Запасной
путь — `git worktree add .claude/worktrees/<имя> -b <ветка> origin/main`.

Чего worktree **не** разводил до 2026-08-19: порты хоста, postgres под pytest,
`node_modules` и venv. Порты теперь разводятся — см. ниже; остальное по-прежнему
общее, и новому worktree нужны свой `npm ci` и скопированные руками
`.env.local` / `.env`, их нет в git.

### Параллельные чаты: свой стек на worktree

`docker-compose.qa.yml` берёт имя проекта и порты из переменных, а без них ведёт
себя как раньше — CI и одиночный чат настраивать не нужно. Второму чату:

```bash
cp .env.qa.local.example .env.qa.local        # поменять номера
docker compose --env-file .env.qa.local -f docker-compose.qa.yml up -d --build --wait
```

`--env-file` нужен **каждой** команде этого стека, включая `down`. Порт бэкенда
запекается во фронт на этапе сборки, поэтому смена `QA_BACKEND_PORT` требует
пересборки образа фронта, а не рестарта: иначе интерфейс второго стека будет
ходить в бэкенд первого — та же тихая поломка, что 2026-08-11, с новой причиной.

Postgres под pytest переменными не разводится: задайте в worktree свой
`DATABASE_URL` с отдельной базой, иначе два прогона делят строки.

**Перед запуском чего-либо внутри контейнера — проверить, чей файл там лежит.**
`./scripts` и `./qa` монтируются из того чекаута, откуда стек поднят:

```bash
docker inspect -f '{{range .Mounts}}{{.Source}} {{end}}' lms-qa-backend-1
```

2026-08-19 это стоило трёх попыток: «мой» скрипт в контейнере оказался файлом
соседней сессии, и правка, которой я его чинил, не исполнялась.

### Параллельные чаты: мержи и прод

- **Мержить по одному.** Дождаться, пока деплой предыдущего мержа завершится,
  прежде чем мержить следующий PR. Два мержа в одно окно оставляют прод на коммит
  позади, и drift-шаг сообщает об этом предупреждением (PR #373).
- **Прод правит один чат за раз.** Чтение параллельно можно. Любая правка данных
  или конфига объявляется в чате до того, как её делают, — иначе два сеанса
  чинят одну строку в разные стороны.

Перед тем как отвести ветку: `git log origin/main..main` должен быть пуст. Если
нет — на `main` висит чужой коммит, и он уедет в твой PR.
