...
async def get_children(db: AsyncSession, parent_id: uuid.UUID) -> list[dict]:
...
async def get_child_progress(
    ...
    """Get a child's learning progress (only if linked)."""
...
async def get_child_grades(
    ...
    """Get a child's assignment grades."""
    link = await db.execute(
        select(ParentChild).where(
            ParentChild.parent_id == parent_id,
            ParentChild.child_id == child_id,
    ...
    result = await db.execute(
        ...
        .where(AssignmentSubmission.student_id == child_id)
