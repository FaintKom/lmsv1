...
These tests poke `parse_answer_set` and `_parse` directly so we don't
...
from app.math_validation.router import _parse
from app.math_validation.service import parse_answer_set
...
class TestAnswerSetParsing:
    """`parse_answer_set` strips the optional "x = " prefix, splits on
    ...
    def test_decimal_answer(self) -> None:
        ...
        assert _str_set(parse_answer_set("x=4.5")) == {"4.50000000000000"}

    def test_integer_answer(self) -> None:
        assert _str_set(parse_answer_set("x=4")) == {"4"}

    def test_bare_number(self) -> None:
        assert _str_set(parse_answer_set("4")) == {"4"}
        assert _str_set(parse_answer_set("4.5")) == {"4.50000000000000"}

    def test_multiple_solutions_or(self) -> None:
        assert _str_set(parse_answer_set("x = 2 or x = 3")) == {"2", "3"}

    def test_multiple_solutions_comma(self) -> None:
        assert _str_set(parse_answer_set("x = 2, x = 3")) == {"2", "3"}

    def test_multiple_solutions_semicolon(self) -> None:
        assert _str_set(parse_answer_set("x=2; x=3")) == {"2", "3"}

    def test_fraction_answer(self) -> None:
        assert _str_set(parse_answer_set("x = 1/2")) == {"1/2"}

    def test_empty_input(self) -> None:
        assert parse_answer_set("") == set()
        assert parse_answer_set("   ") == set()
    ...
    def test_unparseable_parts_skipped_not_raised(self) -> None:
        ...
        result = parse_answer_set("x = 2 or x = banana(garbage")
